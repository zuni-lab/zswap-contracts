...
impl Contract {
    ...
    pub fn collect_approved_tokens_to_mint(
        ...
        let transfer_token_0_promise = ext_ft_core
            ::ext(token_0)
            .ft_transfer(env::predecessor_account_id(), amount_0, None);
        let transfer_token_1_promise = ext_ft_core
            ::ext(token_1)
            .ft_transfer(env::predecessor_account_id(), amount_1, None);
    ...
    pub fn calculate_liquidity(
        ...
        ext_zswap_pool
            ::ext(pool)
            .mint(recipient, params.lower_tick, params.upper_tick, U128::from(liquidity), data)
            .then(
                Self::ext(env::current_account_id()).manager_mint_callback(
                    params.amount_0_min.0,
                    params.amount_1_min.0
                )
            )
    }
