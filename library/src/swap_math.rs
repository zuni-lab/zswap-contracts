...
mod tests {
  ...
  fn test_compute_swap_step() {
    // exact amount in that gets capped at price target in one for zero
    {
      ...
      let price_after_whole_input_amount = get_next_sqrt_price_from_input(price, liquidity.as_u128(), amount.as_u256(), zero_for_one);
    ...
    // exact amount out that gets capped at price target in one for zero
    {
      ...
      let price_after_whole_output_amount = get_next_sqrt_price_from_output(price, liquidity.as_u128(), (amount * I256::from(-1)).as_u256(), zero_for_one);
    ...
    // exact amount in that is fully spent in one for zero
    {
      ...
      let price_after_whole_input_amount_less_fee = get_next_sqrt_price_from_input(price, liquidity.as_u128(), amount.sub(fee_amount.as_i256()).as_u256(), zero_for_one);
    ...
    // amount out is capped at the desired amount out
    {
      ...
      assert_eq!(sqrt_q, U256::from_str("417332158212080721273783715441581").unwrap());
    }
    ...
    // target price of 1 uses partial input amount
    {
      ...
      assert!(amount_in.add(fee_amount) <= U256::from_str("3915081100057732413702495386755767").unwrap());
