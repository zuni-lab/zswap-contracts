...
use crate::math::SqrtPriceMath;
use crate::math::SqrtPriceMathTrait;
use crate::num160::To160;
use super::num160::{U160, I160};
use crate::num24::To24;
use super::num24::{U24, I24};
...
impl SwapMathTrait for SwapMath {
  ...
  fn compute_swap_step(
    ...
    let mut fee_amount: U256 = U256::ZERO;

    if exact_in {
      let amount_remaining_less_fee = FullMath::mul_div(amount_remaining.as_u256(), U256::new((1_000_000 - fee_pips) as u128), 1_000_000.as_u256());
      amount_in = if zero_for_one {
        SqrtPriceMath::get_amount0_delta(sqrt_ratio_target_x96, sqrt_ratio_current_x96, liquidity, true)
      } else {
        SqrtPriceMath::get_amount1_delta(sqrt_ratio_current_x96, sqrt_ratio_target_x96, liquidity, true)
      };
      ...
      amount_out = if zero_for_one {
        SqrtPriceMath::get_amount1_delta(sqrt_ratio_target_x96, sqrt_ratio_current_x96, liquidity, false)
      } else {
        SqrtPriceMath::get_amount0_delta(sqrt_ratio_current_x96, sqrt_ratio_target_x96, liquidity, false)
      };
    ...
    if exact_in && sqrt_ratio_next_x96 != sqrt_ratio_target_x96 {
      ...
      fee_amount = FullMath::mul_div(amount_in, fee_pips.as_u256(), (1_000_000 - fee_pips).as_u256());
    }
